        ...
        <div className="hidden md:flex items-center gap-6">
          <ul className="flex gap-6 text-[#DABF70] font-medium">
            {navLinks.map(({ name, href }) => (
              <li key={name}>
                <Link href={href}>
                  <span className="relative cursor-pointer text-[#DABF70] transition-colors hover:text-white hover:underline">
                    {name}
                  </span>
                </Link>
              </li>
          ...
          <button className="ml-4 bg-white text-black px-4 py-2 rounded-4xl hover:bg-gray-300 transition cursor-pointer">
            Get Started
          </button>
        </div>
        ...
        <div className="md:hidden bg-black text-white py-4 px-6 space-y-4 transition-all duration-300">
          {navLinks.map(({ name, href }) => (
            <Link key={name} href={href} onClick={closeMenu}>
              <div className="block cursor-pointer hover:text-gray-300 transition-colors">
                {name}
              </div>
            </Link>
          ))}
          <button
            onClick={closeMenu}
            className="w-full bg-white text-black px-4 py-2 rounded hover:bg-gray-300 transition cursor-pointer"
          >
            Get Started
          </button>
        </div>
